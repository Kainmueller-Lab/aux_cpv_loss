...
def unet_ext(
    ...
    if batch_norm:
        f_left = tf.layers.batch_normalization(f_left, training=is_training)

    if dropout and dropout != 0.0:
        f_left = tf.layers.dropout(f_left, training=is_training)
    ...
    if batch_norm:
        f_out = tf.layers.batch_normalization(f_out, training=is_training)
