...
def predict(**kwargs):
    ...
    request.add(pred_cp, output_shape_world)
    ...
    zf.create('volumes/pred_cp',
              shape=[1] + list(shape),
              chunks=[1] + list(shape),
              dtype=np.float32)
    zf['volumes/pred_cp'].attrs['offset'] = [0, 0, 0]
    zf['volumes/pred_cp'].attrs['resolution'] = kwargs['voxel_size']
