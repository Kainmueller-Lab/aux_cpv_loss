...
def predict(**kwargs):
    ...
    request.add(pred_sdt, output_shape_world)
    ...
    zf.create('volumes/pred_sdt',
              shape=[1] + list(shape),
              chunks=[1] + list(shape),
              dtype=np.float32)
    zf['volumes/pred_sdt'].attrs['offset'] = [0, 0, 0]
    zf['volumes/pred_sdt'].attrs['resolution'] = kwargs['voxel_size']
